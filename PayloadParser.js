function parseUplink(device, payload)
{
    ...
    // Store 
    /*if(parsed.Temp_Channel1 != null){
        var tc1 = device.endpoints.byAddress("1");

        if (tc1 != null)
            tc1.updateTemperatureSensorStatus(parsed.Temp_Channel1);
    };
    
    if(parsed.Temp_Channel2 != null){
        var tc2 = device.endpoints.byAddress("2");

        if (tc2 != null)
            tc2.updateTemperatureSensorStatus(parsed.Temp_Channel2);
    };
